...
cls
echo KMS通用激活批处理
echo KMS激活源代码：hnfeng 
echo 更新和维护：qpi
echo 正在检查与激活服务器的连接情况……
set KMSSER=www.lidong.xin
echo.
ping www.lidong.xin | find "超时"  > NUL &&  goto fail
ping www.lidong.xin | find "目标主机"  > NUL &&  goto fail
ping www.lidong.xin | find "无法访问"  > NUL &&  goto fail
ping www.lidong.xin | find "故障"  > NUL &&  goto fail
...
echo 检查是否存在office 2016零售版，将执行retail转vol。
...
if DEFINED _Office16Path (echo.&echo 已发现 Office2016 零售版
    call :ActOffice 16 Office2016
  ) else (echo.&echo 未发现 Office2016零售版)
echo 操作成功完成！
echo.&pause
exit
...
echo ---------------------------------------------------
echo 请输入KMS激活服务器地址：
echo 直接回车则使用备用服务器54.223.212.31。
echo 当然你也可以使用其他激活服务器，但批处理不检查其有效性
